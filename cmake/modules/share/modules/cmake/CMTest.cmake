...
function(cm_create_internal_targets NAME)
    if (NOT TARGET ${internal-lib-name})
        file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/${internal-lib-name}.cpp "")
        add_library(${internal-lib-name} STATIC ${CMAKE_CURRENT_BINARY_DIR}/${internal-lib-name}.cpp)
        cm_mark_as_test(${internal-lib-name})
    endif ()
endfunction()
...
function(cm_test_link_libraries)
    ...
    if (BUILD_TESTS)
        ...
        target_link_libraries(${internal-lib-name} ${ARGN})
    else ()
        foreach (TARGET ${ARGN})
            if (TARGET ${TARGET})
                set_property(DIRECTORY APPEND PROPERTY CM_TEST_DEPENDENCIES ${TARGET})
                target_link_libraries(${internal-lib-name} ${TARGET})
            elseif (${TARGET} MATCHES "::")
                ...
                target_link_libraries(${internal-lib-name} $<${HAS_TARGET}:${TARGET}>)
            else ()
                set_property(DIRECTORY APPEND PROPERTY CM_TEST_DEPENDENCIES ${TARGET})
                target_link_libraries(${internal-lib-name} ${TARGET})
            endif ()
            if (BUILD_SHARED_LIBS)
                target_compile_definitions(${internal-lib-name} PRIVATE -DBOOST_TEST_DYN_LINK=1 -DBOOST_TEST_NO_AUTO_LINK=1)
            endif ()
        endforeach ()
    endif ()
endfunction()
...
macro(cm_add_test_subdirectory)
    ...
    if (_enable_tests_property_upper STREQUAL "OFF" OR _enable_tests_property_upper)

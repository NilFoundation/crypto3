...
template <>
struct dbl_bitlen<uint8_t>
{
   using type = uint16_t;
};
template <>
struct dbl_bitlen<uint16_t>
{
   using type = uint32_t;
};
template <>
struct dbl_bitlen<uint32_t>
{
   using type = uint64_t;
};
template <>
struct dbl_bitlen<uint64_t>
{
...
{
   constexpr const std::size_t Bits = result.size();
   std::size_t                 pos  = Bits / 3 + 1;

   //gen_tuple_type tuple_res;

   Backend t(result), t2(result), r(result);
...
constexpr Backend eval_montgomery_reduce_compile_time_backend(Backend result)
{
   ...
   using gen_type = typename generate_tuple_type<int, Bits / 3 + 1>::type;
   gen_type tuple_res;
   ...
   result         = convert_to_backend<Bits / 3 + 1>(gen_tuple);
...
class montgomery_params<cpp_int_backend<Bits, Bits, SignType, Checked, void>> :

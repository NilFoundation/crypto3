...
namespace nil {
    namespace crypto3 {
        ...
        OutputIterator encrypt(InputIterator first, InputIterator last, const KeyPassRange &key,
                               OutputIterator out) {
        ...
        block::detail::range_cipher_impl<block::detail::value_cipher_impl<CipherAccumulator>>
            encrypt(InputIterator first, InputIterator last, const KeyPassRange &key) {
            ...
            return EncrypterImpl(first, last, CipherAccumulator(EncryptionMode(BlockCipher(block::detail::key_value<BlockCipher>(key)))));
        }
        ...
        OutputIterator encrypt(const SinglePassRange &rng, const KeyPassRange &key, OutputIterator out) {
            ...
            return EncrypterImpl(rng, std::move(out), CipherAccumulator(EncryptionMode(BlockCipher(block::detail::key_value<BlockCipher>(key)))));
        }

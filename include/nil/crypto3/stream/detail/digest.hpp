...
#ifndef CRYPTO3_STREAM_DIGEST_HPP
#define CRYPTO3_STREAM_DIGEST_HPP
...
#include <nil/crypto3/stream/detail/pack.hpp>
...
namespace std {
    template<std::size_t DigestBits>
    std::string to_string(const nil::crypto3::stream::digest<DigestBits> &d) {
        std::array<char, DigestBits / 4 + 1> cstr = nil::crypto3::stream::detail::c_str(d);

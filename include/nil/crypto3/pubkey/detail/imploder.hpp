#ifndef CRYPTO3_PUBKEY_DETAIL_IMPLODER_HPP
#define CRYPTO3_PUBKEY_DETAIL_IMPLODER_HPP

#include <nil/crypto3/pubkey/stream_endian.hpp>
#include <nil/crypto3/pubkey/detail/unbounded_shift.hpp>
...
// so we're taking many smaller values and combining them into one value
            ...
            static void step(InputValue z, OutputValue &x) {
                ...
                int const ku = k - kb;
                int const shift =
                        InputBits >= UnitBits ? k : OutputBits >= UnitBits ? ku + (UnitBits - (InputBits + kb)) :
        ...
        static void step(InputValue z, OutputValue &x) {
            ...
            int const ku = k - kb;
            int const shift =
                    InputBits >= UnitBits ? OutputBits - (InputBits + k) : OutputBits >= UnitBits ? OutputBits -
                                                                                                    (UnitBits + ku) + kb
                                                                                                  : kb;
            OutputValue y = low_bits<InputBits>(OutputValue(z));
            x |= unbounded_shl<shift>(y);
...
#endif // CRYPTO3_PUBKEY_DETAIL_IMPLODER_HPP

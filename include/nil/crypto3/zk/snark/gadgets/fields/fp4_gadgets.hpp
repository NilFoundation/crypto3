        ...
        namespace zk {
            namespace snark {
                ...
                public:
                    ...
                    Fp4_variable(protoboard<FieldType> &pb);
                    Fp4_variable(protoboard<FieldType> &pb, const Fp4T &el);
                    Fp4_variable(protoboard<FieldType> &pb, const Fp2_variable<Fp2T> &c0, const Fp2_variable<Fp2T> &c1);
                ...
                public:
                    ...
                    Fp4_tower_mul_gadget(protoboard<FieldType> &pb,
                ...
                public:
                    ...
                    pb_variable<FieldType> v2;
                    pb_variable<FieldType> v6;

                    Fp4_direct_mul_gadget(protoboard<FieldType> &pb,
                                          const Fp4_variable<Fp4T> &A,
                                          const Fp4_variable<Fp4T> &B,
                                          const Fp4_variable<Fp4T> &result);
                    void generate_r1cs_constraints();
                ...
                public:
                    ...
                    Fp4_sqr_gadget(protoboard<FieldType> &pb,
                                   const Fp4_variable<Fp4T> &A,
                                   const Fp4_variable<Fp4T> &result);
                    void generate_r1cs_constraints();
                    void generate_r1cs_witness();
                ...
                public:
                    ...
                    Fp4_cyclotomic_sqr_gadget(protoboard<FieldType> &pb,
                                              const Fp4_variable<Fp4T> &A,
                                              const Fp4_variable<Fp4T> &result);
                    void generate_r1cs_constraints();
                    void generate_r1cs_witness();
                ...
                template<typename Fp4T>
                Fp4_variable<Fp4T>::Fp4_variable(protoboard<FieldType> &pb) : gadget<FieldType>(pb), c0(pb), c1(pb) {
                }

                template<typename Fp4T>
                Fp4_variable<Fp4T>::Fp4_variable(protoboard<FieldType> &pb, const Fp4T &el) :
                    gadget<FieldType>(pb), c0(pb, el.c0), c1(pb, el.c1) {
                }

                template<typename Fp4T>
                Fp4_variable<Fp4T>::Fp4_variable(protoboard<FieldType> &pb,
                ...
                template<typename Fp4T>
                Fp4_tower_mul_gadget<Fp4T>::Fp4_tower_mul_gadget(protoboard<FieldType> &pb,
                                                                 const Fp4_variable<Fp4T> &A,
                                                                 const Fp4_variable<Fp4T> &B,
                                                                 const Fp4_variable<Fp4T> &result) :
                    gadget<FieldType>(pb),
                    ...
                    */
                    ...
                    compute_v1.reset(new Fp2_mul_gadget<Fp2T>(pb, A.c1, B.c1, *v1));
                    ...
                    compute_v0.reset(new Fp2_mul_gadget<Fp2T>(pb, A.c0, B.c0, *v0));
                    ...
                    compute_result_c1.reset(
                        new Fp2_mul_gadget<Fp2T>(pb, *Ac0_plus_Ac1, *Bc0_plus_Bc1, *result_c1_plus_v0_plus_v1));
                }
                ...
                template<typename Fp4T>
                Fp4_direct_mul_gadget<Fp4T>::Fp4_direct_mul_gadget(protoboard<FieldType> &pb,
                                                                   const Fp4_variable<Fp4T> &A,
                                                                   const Fp4_variable<Fp4T> &B,
                                                                   const Fp4_variable<Fp4T> &result) :
                    gadget<FieldType>(pb),
                ...
                template<typename Fp4T>
                Fp4_sqr_gadget<Fp4T>::Fp4_sqr_gadget(protoboard<FieldType> &pb,
                                                     const Fp4_variable<Fp4T> &A,
                                                     const Fp4_variable<Fp4T> &result) :
                    gadget<FieldType>(pb),
                    A(A), result(result) {
                    ...
                    */
                    v1.reset(new Fp2_variable<Fp2T>(pb));
                    compute_v1.reset(new Fp2_sqr_gadget<Fp2T>(pb, A.c1, *v1));

                    v0_c0.assign(pb, result.c0.c0 - Fp4T::non_residue * v1->c1);
                    v0_c1.assign(pb, result.c0.c1 - v1->c0);
                    v0.reset(new Fp2_variable<Fp2T>(pb, v0_c0, v0_c1));

                    compute_v0.reset(new Fp2_sqr_gadget<Fp2T>(pb, A.c0, *v0));
                    ...
                    compute_result_c1.reset(new Fp2_sqr_gadget<Fp2T>(pb, *Ac0_plus_Ac1, *result_c1_plus_v0_plus_v1));
                }
                ...
                template<typename Fp4T>
                Fp4_cyclotomic_sqr_gadget<Fp4T>::Fp4_cyclotomic_sqr_gadget(protoboard<FieldType> &pb,
                                                                           const Fp4_variable<Fp4T> &A,
                                                                           const Fp4_variable<Fp4T> &result) :
                    gadget<FieldType>(pb),
                    A(A), result(result) {

        ...
        namespace zk {
            namespace snark {
                ...
                public:
                    ...
                    Fp6_variable(protoboard<FieldType> &pb);
                    Fp6_variable(protoboard<FieldType> &pb, const Fp6T &el);
                    Fp6_variable(protoboard<FieldType> &pb, const Fp3_variable<Fp3T> &c0, const Fp3_variable<Fp3T> &c1);
                ...
                public:
                    ...
                    Fp6_mul_gadget(protoboard<FieldType> &pb,
                ...
                public:
                    ...
                    Fp6_mul_by_2345_gadget(protoboard<FieldType> &pb,
                                           const Fp6_variable<Fp6T> &A,
                                           const Fp6_variable<Fp6T> &B,
                                           const Fp6_variable<Fp6T> &result);
                    void generate_r1cs_constraints();
                ...
                public:
                    ...
                    Fp6_sqr_gadget(protoboard<FieldType> &pb,
                                   const Fp6_variable<Fp6T> &A,
                                   const Fp6_variable<Fp6T> &result);
                    void generate_r1cs_constraints();
                    void generate_r1cs_witness();
                ...
                public:
                    ...
                    Fp6_cyclotomic_sqr_gadget(protoboard<FieldType> &pb,
                                              const Fp6_variable<Fp6T> &A,
                                              const Fp6_variable<Fp6T> &result);
                    void generate_r1cs_constraints();
                    void generate_r1cs_witness();
                ...
                template<typename Fp6T>
                Fp6_variable<Fp6T>::Fp6_variable(protoboard<FieldType> &pb) : gadget<FieldType>(pb), c0(pb), c1(pb) {
                }

                template<typename Fp6T>
                Fp6_variable<Fp6T>::Fp6_variable(protoboard<FieldType> &pb, const Fp6T &el) :
                    gadget<FieldType>(pb), c0(pb, el.c0), c1(pb, el.c1) {
                }

                template<typename Fp6T>
                Fp6_variable<Fp6T>::Fp6_variable(protoboard<FieldType> &pb,
                ...
                template<typename Fp6T>
                Fp6_mul_gadget<Fp6T>::Fp6_mul_gadget(protoboard<FieldType> &pb,
                                                     const Fp6_variable<Fp6T> &A,
                                                     const Fp6_variable<Fp6T> &B,
                                                     const Fp6_variable<Fp6T> &result) :
                    gadget<FieldType>(pb),
                ...
                template<typename Fp6T>
                Fp6_mul_by_2345_gadget<Fp6T>::Fp6_mul_by_2345_gadget(protoboard<FieldType> &pb,
                                                                     const Fp6_variable<Fp6T> &A,
                                                                     const Fp6_variable<Fp6T> &B,
                                                                     const Fp6_variable<Fp6T> &result) :
                    gadget<FieldType>(pb),
                ...
                template<typename Fp6T>
                Fp6_sqr_gadget<Fp6T>::Fp6_sqr_gadget(protoboard<FieldType> &pb,
                                                     const Fp6_variable<Fp6T> &A,
                                                     const Fp6_variable<Fp6T> &result) :
                    gadget<FieldType>(pb),
                    A(A), result(result) {
                ...
                template<typename Fp6T>
                Fp6_cyclotomic_sqr_gadget<Fp6T>::Fp6_cyclotomic_sqr_gadget(protoboard<FieldType> &pb,
                                                                           const Fp6_variable<Fp6T> &A,
                                                                           const Fp6_variable<Fp6T> &result) :
                    gadget<FieldType>(pb),
                    A(A), result(result) {
                    ...
                    */
                    ...
                    asq_c0.assign(pb, (result.c0.c0 + 2 * a->c0) * FieldType(3).inverse());
                    asq_c1.assign(pb, (result.c1.c1 - 2 * a->c1) * FieldType(3).inverse());

                    bsq_c0.assign(pb, (result.c0.c1 + 2 * c->c0) * FieldType(3).inverse());
                    bsq_c1.assign(pb, (result.c1.c2 - 2 * c->c1) * FieldType(3).inverse());

                    csq_c0.assign(pb, (result.c0.c2 + 2 * b->c1) * FieldType(3).inverse());
                    csq_c1.assign(pb, (result.c1.c0 - 2 * b->c0) * (FieldType(3) * Fp2T::non_residue).inverse());

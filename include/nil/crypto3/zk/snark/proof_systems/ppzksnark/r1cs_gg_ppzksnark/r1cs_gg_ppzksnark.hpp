    ...
    namespace crypto3 {
        namespace zk {
            namespace snark {
                ...
                class r1cs_gg_ppzksnark_proving_key {
                public:
                    algebra::G1<CurveType> alpha_g1;
                    algebra::G1<CurveType> beta_g1;
                    algebra::G2<CurveType> beta_g2;
                    algebra::G1<CurveType> delta_g1;
                    algebra::G2<CurveType> delta_g2;

                    algebra::G1_vector<CurveType> A_query;    // this could be a sparse vector if we had multiexp for those
                    knowledge_commitment_vector<algebra::G2<CurveType>, algebra::G1<CurveType>> B_query;
                    algebra::G1_vector<CurveType> H_query;
                    algebra::G1_vector<CurveType> L_query;
                    ...
                    std::size_t size_in_bits() const {
                        return (algebra::size_in_bits(A_query) + B_query.size_in_bits() +
                                ...
                                1 * algebra::G1<CurveType>::size_in_bits() + 1 * algebra::G2<CurveType>::size_in_bits());
                    }
                ...
                public:
                    ...
                    accumulation_vector<algebra::G1<CurveType>> gamma_ABC_g1;
                    ...
                    r1cs_gg_ppzksnark_verification_key(const algebra::GT<CurveType> &alpha_g1_beta_g2,
                                                       ...
                                                       const algebra::G2<CurveType> &delta_g2,
                                                       const accumulation_vector<algebra::G1<CurveType>> &gamma_ABC_g1) :
                    ...
                    std::size_t size_in_bits() const {
                        ...
                        return (gamma_ABC_g1.size_in_bits() + 2 * algebra::G2<CurveType>::size_in_bits());
                    }
                ...
                class r1cs_gg_ppzksnark_proof {
                public:
                    algebra::G1<CurveType> g_A;
                    algebra::G2<CurveType> g_B;
                    algebra::G1<CurveType> g_C;

                    r1cs_gg_ppzksnark_proof() {
                        // invalid proof with valid curve points
                        this->g_A = algebra::G1<CurveType>::one();
                        this->g_B = algebra::G2<CurveType>::one();
                        this->g_C = algebra::G1<CurveType>::one();
                    }
                    r1cs_gg_ppzksnark_proof(algebra::G1<CurveType> &&g_A, algebra::G2<CurveType> &&g_B, algebra::G1<CurveType> &&g_C) :
                ...
                */
                ...
                r1cs_gg_ppzksnark_verification_key<CurveType>
                    ...
                    result.gamma_g2 = algebra::G2<CurveType>::random_element();
                    result.delta_g2 = algebra::G2<CurveType>::random_element();

                    algebra::G1<CurveType> base = algebra::G1<CurveType>::random_element();
                    algebra::G1_vector<CurveType> v;
                    for (std::size_t i = 0; i < input_size; ++i) {
                        v.emplace_back(algebra::G1<CurveType>::random_element());
                    }

                    result.gamma_ABC_g1 = accumulation_vector<algebra::G1<CurveType>>(std::move(base), std::move(v));
                ...
                r1cs_gg_ppzksnark_keypair<CurveType>
                    ...
                    const algebra::G1<CurveType> g1_generator = algebra::G1<CurveType>::random_element();
                    ...
                    const std::size_t g1_window_size = algebra::get_exp_window_size<algebra::G1<CurveType>>(g1_scalar_count);

                    algebra::window_table<algebra::G1<CurveType>> g1_table =
                        algebra::get_window_table(g1_scalar_size, g1_window_size, g1_generator);

                    const algebra::G2<CurveType> G2_gen = algebra::G2<CurveType>::random_element();
                    ...
                    std::size_t g2_window_size = algebra::get_exp_window_size<algebra::G2<CurveType>>(g2_scalar_count);

                    algebra::window_table<algebra::G2<CurveType>> g2_table =
                        algebra::get_window_table(g2_scalar_size, g2_window_size, G2_gen);

                    algebra::G1<CurveType> alpha_g1 = alpha * g1_generator;
                    algebra::G1<CurveType> beta_g1 = beta * g1_generator;
                    algebra::G2<CurveType> beta_g2 = beta * G2_gen;
                    algebra::G1<CurveType> delta_g1 = delta * g1_generator;
                    algebra::G2<CurveType> delta_g2 = delta * G2_gen;

                    algebra::G1_vector<CurveType> A_query = batch_exp(g1_scalar_size, g1_window_size, g1_table, At);
                    ...
                    algebra::G1_vector<CurveType> L_query = batch_exp(g1_scalar_size, g1_window_size, g1_table, Lt);
#ifdef USE_MIXED_ADDITION
                    algebra::batch_to_special<algebra::G1<CurveType>>(L_query);
#endif

                    algebra::GT<CurveType> alpha_g1_beta_g2 = CurveType::reduced_pairing(alpha_g1, beta_g2);
                    algebra::G2<CurveType> gamma_g2 = gamma * G2_gen;

                    algebra::G1<CurveType> gamma_ABC_g1_0 = gamma_ABC_0 * g1_generator;
                    algebra::G1_vector<CurveType> gamma_ABC_g1_values =
                        batch_exp(g1_scalar_size, g1_window_size, g1_table, gamma_ABC);

                    accumulation_vector<algebra::G1<CurveType>> gamma_ABC_g1(std::move(gamma_ABC_g1_0),
                ...
                r1cs_gg_ppzksnark_proof<CurveType>
                    ...
                    algebra::G1<CurveType> evaluation_At =
                        algebra::multi_exp_with_mixed_addition<algebra::G1<CurveType>, typename CurveType::scalar_field_type,
                            ...
                            chunks);
                    knowledge_commitment<algebra::G2<CurveType>, algebra::G1<CurveType>> evaluation_Bt =
                        kc_multi_exp_with_mixed_addition<algebra::G2<CurveType>, algebra::G1<CurveType>, typename CurveType::scalar_field_type,
                                                         algebra::multi_exp_method_BDLO12>(
                            pk.B_query,
                            0,
                            ...
                            const_padded_assignment.begin() + qap_wit.num_variables() + 1,
                            chunks);
                    algebra::G1<CurveType> evaluation_Ht =
                        algebra::multi_exp<algebra::G1<CurveType>, typename CurveType::scalar_field_type, algebra::multi_exp_method_BDLO12>(
                            ...
                            chunks);
                    algebra::G1<CurveType> evaluation_Lt =
                        algebra::multi_exp_with_mixed_addition<algebra::G1<CurveType>, typename CurveType::scalar_field_type,
                                                               algebra::multi_exp_method_BDLO12>(
                            pk.L_query.begin(),
                            pk.L_query.end(),
                    ...
                    algebra::G1<CurveType> g1_A = pk.alpha_g1 + evaluation_At + r * pk.delta_g1;

                    /* B = beta + sum_i(a_i*B_i(t)) + s*delta */
                    algebra::G1<CurveType> g1_B = pk.beta_g1 + evaluation_Bt.h + s * pk.delta_g1;
                    algebra::G2<CurveType> g2_B = pk.beta_g2 + evaluation_Bt.g + s * pk.delta_g2;

                    /* C = sum_i(a_i*((beta*A_i(t) + alpha*B_i(t) + C_i(t)) + H(t)*Z(t))/delta) + A*s + r*b - r*s*delta
                     */
                    algebra::G1<CurveType> g1_C = evaluation_Ht + evaluation_Lt + s * g1_A + r * g1_B - (r * s) * pk.delta_g1;

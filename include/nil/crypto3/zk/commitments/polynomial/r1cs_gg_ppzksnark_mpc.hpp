            ...
            namespace commitments {
                ...
                public:
                    ...
                    static private_key_type generate_private_key() {
                        typename scalar_field_type::value_type delta =
                    ...
                    static public_key_type proof_eval(const private_key_type &private_key,
                        ...
                        std::vector<std::uint8_t> transcript = compute_transcript(mpc_keypair.first.constraint_system, previous_public_key);
                        auto delta_pok = proof_of_knowledge_scheme_type::proof_eval(private_key.delta, transcript, 0);
                        g1_value_type delta_after = private_key.delta * (previous_public_key ? previous_public_key->delta_after : g1_value_type::one());
                        return public_key_type {
                            std::move(delta_after),
                            std::move(delta_pok)
                        };
                    }
                    
                    static bool verify_eval(const proving_scheme_keypair_type &mpc_keypair, const std::vector<public_key_type> &pubkeys,
                                            const constraint_system_type &constraint_system,
                                            const detail::powers_of_tau_result<curve_type> &powers_of_tau_result) {
                        auto initial_keypair = detail::make_r1cs_gg_ppzksnark_keypair_from_powers_of_tau(constraint_system, powers_of_tau_result);
                        ...
                        for(auto pk : pubkeys) {
                            auto g2_s = proof_of_knowledge_scheme_type::compute_g2_s(pk.delta_pok.g1_s, pk.delta_pok.g1_s_x, transcript, 0);
                    ...
                    static std::vector<std::uint8_t> compute_transcript(const constraint_system_type &constraint_system,
                    ...
                    static std::vector<std::uint8_t> serialize_public_key(const public_key_type &pubkey) {
                        ...
                        auto filled_val = nil::crypto3::marshalling::types::fill_r1cs_gg_ppzksnark_mpc_public_key<public_key_type, endianness>(pubkey);
                    ...
                    static std::vector<std::uint8_t> serialize_constraint_system(const constraint_system_type &constraint_system) {
                        using endianness = nil::marshalling::option::little_endian;
                        auto filled_val = nil::crypto3::marshalling::types::fill_r1cs_constraint_system<constraint_system_type, endianness>(constraint_system);
                        std::vector<std::uint8_t> blob(filled_val.length());
                        auto it = std::begin(blob);
                        nil::marshalling::status_type status = filled_val.write(it, blob.size());
                        BOOST_ASSERT(status == nil::marshalling::status_type::success);

            ...
            namespace commitments {
                ...
                public:
                    ...
                    static public_key_type proof_eval(const private_key_type &private_key,
                        ...
                        auto tau_pok = proof_of_knowledge_scheme_type::proof_eval(private_key.tau, transcript, tau_personalization, rng);
                        auto alpha_pok = proof_of_knowledge_scheme_type::proof_eval(private_key.alpha, transcript, alpha_personalization, rng);
                        auto beta_pok = proof_of_knowledge_scheme_type::proof_eval(private_key.beta, transcript, beta_personalization, rng);
                    ...
                    static bool verify_eval(const public_key_type &public_key,
                        ...
                        auto tau_g2_s = proof_of_knowledge_scheme_type::compute_g2_s(public_key.tau_pok.g1_s, public_key.tau_pok.g1_s_x, transcript, tau_personalization);
                        auto alpha_g2_s = proof_of_knowledge_scheme_type::compute_g2_s(public_key.alpha_pok.g1_s, public_key.alpha_pok.g1_s_x, transcript, alpha_personalization);
                        auto beta_g2_s = proof_of_knowledge_scheme_type::compute_g2_s(public_key.beta_pok.g1_s, public_key.beta_pok.g1_s_x, transcript, beta_personalization);
                        ...
                        if(!is_same_ratio(detail::power_pairs<scalar_field_type>(after.tau_powers_g1), std::make_pair(after.tau_powers_g2[0], after.tau_powers_g2[1]))) {
                            return false;
                        }
                        if(!is_same_ratio(std::make_pair(after.tau_powers_g1[0], after.tau_powers_g1[1]), commitments::detail::power_pairs<scalar_field_type>(after.tau_powers_g2))) {
                            return false;
                        }
                        if(!is_same_ratio(detail::power_pairs<scalar_field_type>(after.alpha_tau_powers_g1), std::make_pair(after.tau_powers_g2[0], after.tau_powers_g2[1]))) {
                            return false;
                        }
                        if(!is_same_ratio(detail::power_pairs<scalar_field_type>(after.beta_tau_powers_g1), std::make_pair(after.tau_powers_g2[0], after.tau_powers_g2[1]))) {
                            return false;
                        }

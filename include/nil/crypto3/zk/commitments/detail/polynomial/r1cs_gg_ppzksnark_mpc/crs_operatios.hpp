        ...
        namespace zk {
            namespace commitments {
                namespace detail {
                    ...
                    typename snark::r1cs_gg_ppzksnark<CurveType>::keypair_type make_r1cs_gg_ppzksnark_keypair_from_powers_of_tau(
                            const typename snark::r1cs_gg_ppzksnark<CurveType>::constraint_system_type &constraint_system,
                        ...
                        BOOST_ASSERT_MSG(powers_of_tau_result.alpha_coeffs_g1.size() == qap.domain->m, "powers_of_tau_result size does not match the constraint system");
                        BOOST_ASSERT_MSG(powers_of_tau_result.beta_coeffs_g1.size() == qap.domain->m, "powers_of_tau_result size does not match the constraint system");
                        BOOST_ASSERT_MSG(powers_of_tau_result.h.size() == qap.domain->m - 1, "powers_of_tau_result size does not match the constraint system");
                        ...
                        for(std::size_t i = 0; i < qap.num_variables + 1; ++i) {
                            for(auto [lag, coeff] : qap.A_in_Lagrange_basis[i]){
                                ...
                                beta_a_alpha_b_c[i] = beta_a_alpha_b_c[i] + coeff * powers_of_tau_result.beta_coeffs_g1[lag];
                            }
                            for(auto [lag, coeff] : qap.B_in_Lagrange_basis[i]){
                                b_kc[i] = b_kc[i] + coeff * kc_value_type(powers_of_tau_result.coeffs_g2[lag], powers_of_tau_result.coeffs_g1[lag]);
                                beta_a_alpha_b_c[i] = beta_a_alpha_b_c[i] + coeff * powers_of_tau_result.alpha_coeffs_g1[lag];
                            }

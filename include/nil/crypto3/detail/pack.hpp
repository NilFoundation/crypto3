...
namespace nil {
    namespace crypto3 {
        ...
        struct packer : real_packer<Endianness, InputBits, OutputBits> {
            ...
            inline static typename std::enable_if<can_memcpy<Endianness, InputBits, OutputBits, InT, OutT>::value>::type
        ...
        template<typename Endianness, int InValueBits, int OutValueBits, typename InputIterator, typename OutputType,
        ...
        inline void pack(InputIterator first, InputIterator last, OutputType &out) {
            pack_n<Endianness, InValueBits, OutValueBits>(first, std::distance(first, last), out.begin(), out.size());
        }

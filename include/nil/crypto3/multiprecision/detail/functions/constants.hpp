...
void calc_log2(T& num, unsigned digits)
{
   ...
   typedef typename boost::mpl::front<typename T::signed_types>::type                         si_type;
...
{
   typedef typename boost::mpl::front<typename T::unsigned_types>::type ui_type;
   typedef typename boost::mpl::front<typename T::float_types>::type    real_type;
   //
   // 1100 digits in string form:
   //
...
const T& get_constant_one_over_epsilon()
{
   ...
   if ((digits != nil::crypto3::multiprecision::detail::digits2<number<T> >::value()))
   {
      ...
      if(std::numeric_limits<number<T> >::is_specialized)
         ...
         eval_ldexp(result, result, nil::crypto3::multiprecision::detail::digits2<number<T> >::value() - 1);
      digits = nil::crypto3::multiprecision::detail::digits2<number<T> >::value();
   }

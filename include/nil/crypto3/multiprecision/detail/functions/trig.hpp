...
void eval_sin(T& result, const T& x)
{
   ...
   case FP_NAN:
      ...
      else BOOST_THROW_EXCEPTION(std::domain_error("Result is undefined or complex and there is no NaN for this number type."));
      return;
   case FP_ZERO:
      result = x;
      return;
...
void eval_asin(T& result, const T& x)
{
   ...
   std::intmax_t target_precision  = std::numeric_limits<number<T> >::is_specialized ? current_precision - 1 - (std::numeric_limits<number<T> >::digits * 2) / 3
...
inline typename std::enable_if<nil::crypto3::multiprecision::detail::is_arithmetic<A>::value, void>::type eval_atan2(T& result, const T& x, const A& a)
{
...
template <class T, class A>
inline typename std::enable_if<nil::crypto3::multiprecision::detail::is_arithmetic<A>::value, void>::type eval_atan2(T& result, const A& x, const T& a)
{
   using canonical_type = typename nil::crypto3::multiprecision::detail::canonical<A, T>::type;
   using cast_type      = typename std::conditional<std::is_same<A, canonical_type>::value, T, canonical_type>::type;
   cast_type c;

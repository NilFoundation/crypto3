...
namespace detail {
    ...
    inline void pow_imp(T& result, const T& t, const U& p, const boost::mpl::false_&) {
        ...
        typedef typename nil::crypto3::multiprecision::detail::canonical<U, T>::type int_type;

        if (&result == &t) {
            T temp;
            pow_imp(temp, t, p, boost::mpl::false_());
            result = temp;
            return;
        }
    ...
    template<typename T, typename U>
    inline void pow_imp(T& result, const T& t, const U& p, const boost::mpl::true_&) {
        // Signed integer power, just take care of the sign then call the unsigned version:
        typedef typename nil::crypto3::multiprecision::detail::canonical<U, T>::type int_type;
        typedef typename boost::make_unsigned<U>::type ui_type;

        if (p < 0) {
            T temp;
            temp = static_cast<int_type>(1);
            T denom;
            pow_imp(denom, t, static_cast<ui_type>(-p), boost::mpl::false_());
            eval_divide(result, temp, denom);
            return;
        }
        pow_imp(result, t, static_cast<ui_type>(p), boost::mpl::false_());
    }

}    // namespace detail

template<typename T, typename U>
inline typename boost::enable_if_c<boost::is_integral<U>::value>::type eval_pow(T& result, const T& t, const U& p) {
...
void hyp1F0(T& H1F0, const T& a, const T& x) {
    ...
    typedef typename nil::crypto3::multiprecision::detail::canonical<int, T>::type si_type;
...
void eval_exp(T& result, const T& x) {
    ...
    typedef typename nil::crypto3::multiprecision::detail::canonical<exp_type, T>::type canonical_exp_type;
    ...
    typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type ll;
    eval_trunc(exp_series, x);
    eval_convert_to(&ll, exp_series);
    if (x.compare(ll) == 0) {
        detail::pow_imp(result, get_constant_e<T>(), ll, boost::mpl::true_());
        return;
    ...
    detail::pow_imp(exp_series, result, p2, boost::mpl::true_());
    result = ui_type(1);
...
void eval_log(T& result, const T& arg) {
    ...
    typedef typename T::exponent_type exp_type;
    typedef typename nil::crypto3::multiprecision::detail::canonical<exp_type, T>::type canonical_exp_type;
    typedef typename boost::mpl::front<typename T::float_types>::type fp_type;
...
const T& get_constant_log10() {
    ...
    if (!b || (digits != nil::crypto3::multiprecision::detail::digits2<number<T>>::value())) {
        b = true;
#else
...
inline void eval_pow(T& result, const T& x, const T& a) {
        ...
        case FP_ZERO:
            switch (eval_fpclassify(a)) {
                ...
                case FP_NORMAL: {
                    ...
                    try {
                        typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type i;
                        eval_convert_to(&i, a);
                        if (a.compare(i) == 0) {
                            if (eval_signbit(a)) {
                                if (i & 1) {
    ...
    typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type an;
    typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type max_an =
        std::numeric_limits<
            typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type>::is_specialized ?
            (std::numeric_limits<typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type>::max)() :
            static_cast<typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type>(1)
                << (sizeof(typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type) * CHAR_BIT - 2);
    typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type min_an =
        std::numeric_limits<
            typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type>::is_specialized ?
            (std::numeric_limits<typename nil::crypto3::multiprecision::detail::canonical<boost::intmax_t, T>::type>::min)() :
            -min_an;
    ...
    try {
        ...
        if (a.compare(an) == 0) {
            detail::pow_imp(result, x, an, boost::mpl::true_());
            return;
        }
#ifndef BOOST_NO_EXCEPTIONS
    } catch (const std::exception&) {
    ...
    if ((eval_get_sign(x) < 0)) {
        typename nil::crypto3::multiprecision::detail::canonical<boost::uintmax_t, T>::type aun;
#ifndef BOOST_NO_EXCEPTIONS
        try {
#endif
    ...
    if ((x.compare(fp_type(0.5)) >= 0) && (x.compare(fp_type(0.9)) < 0) && (an < max_an) && (an > min_an)) {
        if (a.compare(fp_type(1e-5f)) <= 0) {
            ...
            if (an) {
                ...
                eval_subtract(t, x);
                hyp1F0(result, da, t);
                detail::pow_imp(t, x, an, boost::mpl::true_());
...
inline typename boost::enable_if_c<!boost::is_integral<A>::value, void>::type
#else
inline typename boost::enable_if_c<is_compatible_arithmetic_type<A, number<T>>::value && !boost::is_integral<A>::value, void>::type
#endif
    ...
    typedef typename boost::mpl::if_<boost::is_same<A, canonical_type>, T, canonical_type>::type cast_type;
    cast_type c;
    c = a;
    eval_pow(result, x, c);
}
...
inline void
#else
inline typename boost::enable_if_c<is_compatible_arithmetic_type<A, number<T>>::value, void>::type
#endif
...
void eval_exp2(T& result, const T& arg) {
    ...
    try {
        ...
        if (arg.compare(i) == 0) {
            temp = static_cast<typename boost::mpl::front<typename T::unsigned_types>::type>(1u);
            eval_ldexp(result, temp, i);
            return;
        }
    } catch (const boost::math::rounding_error&) { /* Fallthrough */
    } catch (const std::runtime_error&) {          /* Fallthrough */
    }

    temp = static_cast<typename boost::mpl::front<typename T::unsigned_types>::type>(2u);
...
inline void eval_tanh(T& result, const T& x) {
    ...
    if ((eval_fpclassify(result) == FP_INFINITE) && (eval_fpclassify(c) == FP_INFINITE)) {
        ...
        result = static_cast<typename boost::mpl::front<typename T::unsigned_types>::type>(1u);
        if (s)
            result.negate();
        return;
    }

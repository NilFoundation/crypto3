...
//
template <class T>
constexpr typename std::enable_if<(nil::crypto3::multiprecision::detail::is_signed<T>::value || std::is_floating_point<T>::value), T>::type abs(T t) noexcept
{
...
template <class T>
constexpr typename std::enable_if<nil::crypto3::multiprecision::detail::is_unsigned<T>::value, T>::type abs(T t) noexcept
{
   return t;
}
...
template <class T>
constexpr typename std::enable_if<(nil::crypto3::multiprecision::detail::is_signed<T>::value || std::is_floating_point<T>::value), typename nil::crypto3::multiprecision::detail::make_unsigned<T>::type>::type unsigned_abs(T t) noexcept
{
   // This strange expression avoids a hardware trap in the corner case
   // that val is the most negative value permitted in boost::long_long_type.
   // See https://svn.boost.org/trac/boost/ticket/9740.
   return t < 0 ? static_cast<typename nil::crypto3::multiprecision::detail::make_unsigned<T>::type>(1u) + static_cast<typename nil::crypto3::multiprecision::detail::make_unsigned<T>::type>(-(t + 1)) : static_cast<typename nil::crypto3::multiprecision::detail::make_unsigned<T>::type>(t);
}
template <class T>
constexpr typename std::enable_if<nil::crypto3::multiprecision::detail::is_unsigned<T>::value, T>::type unsigned_abs(T t) noexcept
{
   return t;
}
...
struct bits_of
{
   static_assert(nil::crypto3::multiprecision::detail::is_integral<T>::value || std::is_enum<T>::value || std::numeric_limits<T>::is_specialized, "Failed integer size check");
...
struct canonical
{
   using tag_type = typename std::conditional<
       nil::crypto3::multiprecision::detail::is_signed<Val>::value && nil::crypto3::multiprecision::detail::is_integral<Val>::value,
...
template <class T>
struct expression_storage : public expression_storage_base<T, nil::crypto3::multiprecision::detail::is_arithmetic<T>::value>
{};
...
{};
template <class Num>
struct number_category : public number_category_base<Num, std::is_class<Num>::value || nil::crypto3::multiprecision::detail::is_arithmetic<Num>::value, std::is_abstract<Num>::value>
{};
template <class Backend, expression_template_option ExpressionTemplates>
...
{
   using type = typename nil::crypto3::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type;
};
...
{
   using val_type = typename nil::crypto3::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type;
   return val_type(val).template convert_to<R>();
}

template <class B, nil::crypto3::multiprecision::expression_template_option ET>
struct is_complex_type<nil::crypto3::multiprecision::number<B, ET> > : public std::integral_constant<bool, nil::crypto3::multiprecision::number_category<B>::value == nil::crypto3::multiprecision::number_kind_complex>
{};

...
class modular_adaptor<modular_fixed_cpp_int_backend<MinBits, SignType, Checked> >
{
 ...
 public:
   ...
   constexpr modular_adaptor(modular_adaptor&& o) : m_base(std::move(o.base_data())), m_mod(std::move(o.mod_data()))
   {
   }
#endif
   ...
   constexpr explicit modular_adaptor(const Backend& m) : m_base(static_cast<typename std::tuple_element<0, unsigned_types>::type>(0u)), m_mod(number_type(m))
   {
      mod_data().adjust_modular(base_data());
   }

   constexpr explicit modular_adaptor(const number_type& m) : m_base(static_cast<typename std::tuple_element<0, unsigned_types>::type>(0u)), m_mod(m)
   {
      mod_data().adjust_modular(base_data());
   }

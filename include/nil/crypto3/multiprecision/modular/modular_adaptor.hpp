...
struct modular_adaptor
{
 ...
 public:
   ...
   modular_adaptor(modular_adaptor&& o) : m_base(std::move(o.base_data())), m_mod(std::move(o.mod_data()))
   {
   }
   ...
   modular_adaptor(const Backend& val) : m_base(val), m_mod(typename std::tuple_element<0, unsigned_types>::type(0u))
   {
   }

   modular_adaptor(const modular_params<Backend>& mod) : m_base(typename std::tuple_element<0, unsigned_types>::type(0u)), m_mod(mod)
   {
   }
...
size_t window_bits(size_t exp_bits)
{
   ...
   BOOST_STATIC_CONSTEXPR size_t wsize[wsize_count][2] = {{1434, 7}, {539, 6}, {197, 4}, {70, 3}, {17, 2}, {0, 0}};

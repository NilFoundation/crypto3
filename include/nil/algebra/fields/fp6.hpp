...
namespace nil {
    namespace algebra {
        ...
        private:
            ...
            using point_fp6 = detail::point<fp6<Modulus>, NumberType>;

        public:
            constexpr fp6(const point_fp6 &point) : top_non_residue(point) {
            }    // init point for non_residue in fp3 or higher

            constexpr fp6(const point_fp &point) : non_residue(fp(point)) {
            ...
            constexpr const point_fp6 top_non_residue;
        ...
        fp6_model<n, modulus> fp6_model<n, modulus>::Frobenius_map(unsigned long power) const {
            return fp6_model<n, modulus>(c0, Frobenius_coeffs_c1[power % 2] * c1);
        }

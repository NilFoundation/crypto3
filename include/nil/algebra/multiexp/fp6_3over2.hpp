...
#include <boost/algebra/fields/detail/fp6_3over2.hpp>

namespace nil {
    namespace algebra {
        namespace fields {
            template<typename NumberType>
            NumberType multi_exp(typename static std::array<fp6_3over2::value_type>::const_iterator vec_start,
                                 typename static std::array<NumberType>::const_iterator scalar_start, size_t num_groups,
                                 const size_t bucket_size, size_t const workers_in_subgroup, const size_t n,
                                 const size_t one) {
            ...
            template<typename NumberType>
            NumberType multi_exp_subgroup(typename static std::array<fp6_3over2::value_type>::const_iterator vec_start,
                ...
                typename std::array<NumberType> part_sum[workers_amount * c] = {[0 ...(workers_amount * c) - 1] = one};
                ...
                for (size_t j = 0; j < workers_amount; ++j) {
                    for (size_t k = 0; k <= c - 1; ++k) {
                        ...
                        for (size_t i = start; i <= end; ++i) {
                            ...
                            if (idx > 0) {
                                buckets[idx - 1] = (*(vec_start + i).data[0].data[0].data) * buckets[idx - 1];
                            }
                        }
            ...
            template<typename NumberType>
            NumberType result_aggregation(typename std::array<NumberType> &r, const size_t L, const size_t bucket_size,
                                          const size_t one) {

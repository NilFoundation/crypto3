...
namespace detail {
    ...
    template<class T>
    inline typename boost::disable_if_c<boost::is_unsigned<T>::value ||
                                            nil::crypto3::multiprecision::is_unsigned_number<T>::value,
                                        T>::type
        abs(const T& a) {
        return a < 0 ? -a : a;
    }
    ...
    inline typename boost::
        enable_if_c<boost::is_unsigned<T>::value || nil::crypto3::multiprecision::is_unsigned_number<T>::value, T>::type
        abs(const T& a) {
        return a;
    }
...
typename boost::disable_if_c<(nil::crypto3::multiprecision::number_category<T>::value ==
                              nil::crypto3::multiprecision::number_kind_integer) ||
                                 nil::crypto3::multiprecision::is_interval_number<T>::value,
                             T>::type
    relative_error(T a, T b) {
    using ::detail::abs;
    using std::abs;
...
typename boost::mpl::if_c<boost::is_convertible<T, U>::value, U, T>::type relative_error(T a, U b) {
...
template<class T>
typename boost::enable_if_c<nil::crypto3::multiprecision::is_interval_number<T>::value, T>::type relative_error(T a,
                                                                                                                T b) {
...
enum { warn_on_fail, error_on_fail, abort_on_fail };
...
inline int digits_of(const T&) {
    return std::numeric_limits<T>::is_specialized ? std::numeric_limits<T>::digits10 + 3 :

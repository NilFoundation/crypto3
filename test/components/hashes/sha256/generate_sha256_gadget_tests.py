...
def bytes_to_words(arr):
    l = len(arr)
    assert l % 4 == 0
    return [(arr[i * 4 + 3] << 24) + (arr[i * 4 + 2] << 16) + (arr[i * 4 + 1] << 8) + arr[i * 4] for i in
            xrange(l // 4)]
...
def generate_sha256_component_tests():
    ...
    print
    "const std::vector<bool> left_bv = %s;" % cpp_val(left)
    print
    "const std::vector<bool> right_bv = %s;" % cpp_val(right)
    print
    "const std::vector<bool> hash_bv = %s;" % cpp_val(hash)

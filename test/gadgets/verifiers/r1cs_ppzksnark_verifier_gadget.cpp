...
void test_verifier(const std::string &annotation_A, const std::string &annotation_B) {
    typedef algebra::Fr<ppT_A> FieldT_A;
    typedef algebra::Fr<ppT_B> FieldT_B;
    ...
    algebra::bit_vector input_as_bits;
    for (const FieldT_A &el : example.primary_input) {
        algebra::bit_vector v = algebra::convert_field_element_to_bit_vector<FieldT_A>(el, elt_size);
...
void test_cyclotomic_sqr(const std::string &annotation) {
    typedef algebra::Fqk<ppT> FpExtT;
    typedef typename FpExtT::my_Fp FieldT;
...
void test_full_pairing(const std::string &annotation) {
    ...
    algebra::Fqk<other_curve<ppT>> native_miller_result =
        other_curve<ppT>::affine_ate_miller_loop(native_prec_P, native_prec_Q);

    algebra::Fqk<other_curve<ppT>> native_finexp_result = other_curve<ppT>::final_exponentiation(native_miller_result);
    printf("Must match:\n");
...
int main() {
    ...
    test_cyclotomic_sqr<algebra::mnt4_pp, Fp4_variable, Fp4_cyclotomic_sqr_gadget>("mnt4_Fp4");
    ...
    test_cyclotomic_sqr<algebra::mnt6_pp, Fp6_variable, Fp6_cyclotomic_sqr_gadget>("mnt6_Fp6");
    ...
    test_G2_checker_gadget<algebra::mnt4_pp>("mnt4");
    test_G2_checker_gadget<algebra::mnt6_pp>("mnt6");

    test_G1_variable_precomp<algebra::mnt4_pp>("mnt4");
    test_G1_variable_precomp<algebra::mnt6_pp>("mnt6");

    test_G2_variable_precomp<algebra::mnt4_pp>("mnt4");
    test_G2_variable_precomp<algebra::mnt6_pp>("mnt6");

    test_mnt_miller_loop<algebra::mnt4_pp>("mnt4");
    test_mnt_miller_loop<algebra::mnt6_pp>("mnt6");

    test_mnt_e_over_e_miller_loop<algebra::mnt4_pp>("mnt4");
    test_mnt_e_over_e_miller_loop<algebra::mnt6_pp>("mnt6");

    test_mnt_e_times_e_over_e_miller_loop<algebra::mnt4_pp>("mnt4");
    test_mnt_e_times_e_over_e_miller_loop<algebra::mnt6_pp>("mnt6");

    test_full_pairing<algebra::mnt4_pp>("mnt4");
    test_full_pairing<algebra::mnt6_pp>("mnt6");

    test_full_precomputed_pairing<algebra::mnt4_pp>("mnt4");
    test_full_precomputed_pairing<algebra::mnt6_pp>("mnt6");

    test_verifier<algebra::mnt4_pp, algebra::mnt6_pp>("mnt4", "mnt6");
    test_verifier<algebra::mnt6_pp, algebra::mnt4_pp>("mnt6", "mnt4");

    test_hardcoded_verifier<algebra::mnt4_pp, algebra::mnt6_pp>("mnt4", "mnt6");
    test_hardcoded_verifier<algebra::mnt6_pp, algebra::mnt4_pp>("mnt6", "mnt4");
}

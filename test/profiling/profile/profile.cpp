...
void profile(const std::string domain_sizes,
    ...
    while (domain_stream >> n)
        dom_sizes.push_back(n);
    ...
    for (size_t s = 0; s < domain.size(); s++) {
        ...
        /* Perform operation */
        if (key == 0)
            basic_radix2_domain<FieldT>(n).FFT(a);
        else if (key == 1)
            extended_radix2_domain<FieldT>(n).FFT(a);
        else if (key == 2)
            step_radix2_domain<FieldT>(n).FFT(a);
        else if (key == 3)
            geometric_sequence_domain<FieldT>(n).FFT(a);
        else if (key == 4)
            arithmetic_sequence_domain<FieldT>(n).FFT(a);

        /* End time */
        double runtime = double(omp_get_wtime() - start);
        if (profiling_type[0])
            runtime_file << n << "," << runtime << "\n";

        /* Memory usage */
        struct rusage r_usage;
        getrusage(RUSAGE_SELF, &r_usage);
        if (profiling_type[1])
            memory_file << n << "," << r_usage.ru_maxrss << "\n";

        /* Operator count */
        if (profiling_type[2] && num_threads == 1)
            operators_file << n << "," << FieldT::add_cnt << "," << FieldT::sub_cnt << "," << FieldT::mul_cnt << ","
                           << FieldT::inv_cnt << "\n";
...
int main(int argc, char* argv[]) {
    ...
    while (profiling_stream >> m)
        profiling_type[m - 1] = 1;
    if (profiling_type[0])
        if (system(("mkdir -p " + path[0]).c_str()))
            return 0;
    if (profiling_type[1])
        if (system(("mkdir -p " + path[1]).c_str()))
            return 0;
    if (profiling_type[2])
        if (system(("mkdir -p " + path[2]).c_str()))
            return 0;

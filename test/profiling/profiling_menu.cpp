    ...
    if (valid_input == 1) {
        ...
        if (valid_input == 1) {
            ...
            do {
                std::cout << "> ";
                getline(std::cin, domain);
            } while (strcmp(domain.c_str(), "1") && strcmp(domain.c_str(), "2") && strcmp(domain.c_str(), "3") &&
            ...
            if (domain_choice >= 0 && domain_choice < 3) {
                ...
                for (int threads = 0; threads < 4; threads++) {
                    ...
                    {
                        if (key > 2 && domain_type == 2)
                            continue;
                        if (key < 3 && domain_type == 3)
                            continue;
                        if (system(("./profiler " + std::to_string(key) + " " + std::to_string(1u << threads) + " " +
                                    datetime + " " + "\"" + profile_type + "\" " + "\"" + domains[domain_choice] + "\"")
                                       .c_str()))
                            printf("\n error: profiling\n");
                    }
                }
...
int main() {
    ...
    while (resume) {
        ...
        do {
            std::cout << "> ";
            getline(std::cin, res);
        } while (strcmp(res.c_str(), "1") && strcmp(res.c_str(), "2") && strcmp(res.c_str(), "3"));

        if (strcmp(res.c_str(), "1") == 0)
            profile();
        else if (strcmp(res.c_str(), "2") == 0)
            plot();
        else if (strcmp(res.c_str(), "3") == 0)
            resume = 0;
    }

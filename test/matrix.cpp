...
static_assert(cast<float>(m1) == matrix {{{1.f, 2.f, 3.f}, {4.f, 5.f, 6.f}, {7.f, 8.f, 9.f}}}, "matrix cast");

static_assert(fill<2, 2>(3.) == matrix {{{3., 3.}, {3., 3.}}}, "matrix fill");

static_assert(transpose(m1) == matrix {{{1., 4., 7.}, {2., 5., 8.}, {3., 6., 9.}}}, "transpose");

static_assert(hermitian(m1) == matrix {{{1., 4., 7.}, {2., 5., 8.}, {3., 6., 9.}}}, "herm-real");

static_assert(hermitian(m1c) == matrix {{{{1., -1.}, {4., -1.}, {7., -1.}},
                                         {{2., 0.}, {5., 0.}, {8., 0.}},
                                         {{3., 0.}, {6., 0.}, {9., 0.}}}},
              "herm-complex");
...
static_assert(identity<std::complex<double>, 3> ==
                  matrix<std::complex<double>, 3, 3> {{{1., 0., 0.}, {0., 1., 0.}, {0., 0., 1.}}},
              "complex identity");

static_assert(repmat<3, 2>(m2) == matrix {{{1., 2., 1., 2.}, {1., 2., 1., 2.}, {1., 2., 1., 2.}}}, "repmat");

static_assert(kron(m2, m2) == matrix {{{1., 2., 2., 4.}}}, "kron");
...
static_assert(reshape<1, 9>(m1) == matrix<double, 1, 9> {{{1., 2., 3., 4., 5., 6., 7., 8., 9.}}}, "reshape");

static_assert(reshape<4, 2>(matrix<double, 2, 4> {{{1., 2., 3., 4.}, {5., 6., 7., 8.}}}) ==
                  matrix<double, 4, 2> {{{1., 2.}, {3., 4.}, {5., 6.}, {7., 8.}}},
              "reshape");

static_assert(real(m1c) == m1, "real");

static_assert(imag(m1c) == matrix {{{1., 0., 0.}, {1., 0., 0.}, {1., 0., 0.}}}, "imag");

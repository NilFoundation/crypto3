...
Division uses the algorithm described in this book: https://members.loria.fr/PZimmermann/mca/pub226.html
...
The non-assembly SIMD integer implementation is in the simd\_integer\_\* files that don't end with "asm". It is only used for testing the assembly code. simd\_integer\_test.cpp has some test code for manual testing.
...
This appends an instruction the ASM code buffer. It also substitutes the macros: APPEND\_M(str( "VPSUBQ `data_size_minus_bits,` data\_size\_minus\_bits, \`bits" ));
...
The following optimizations are used: For normalize and reduce, it is possible to reduce the number of operations if the remainder is used in addition to the quotient.

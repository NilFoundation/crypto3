...
bool process_prover_command_line(const int argc, const char **argv, std::string &processed_assembly_fn,
                                 std::string &proving_key_fn, std::string &primary_input_fn,
                                 std::string &auxiliary_input_fn, std::string &proof_fn) {
    try {
...
int main(int argc, const char *argv[]) {
    ...
    if (!process_prover_command_line(argc, argv, processed_assembly_fn, proving_key_fn, primary_input_fn,
                                     auxiliary_input_fn, proof_fn)) {
        return 1;
    }
#endif
    ...
    const ram_boot_trace<default_tinyram_ppzksnark_pp> boot_trace =
        tinyram_boot_trace_from_program_and_input(pk.ap, pk.primary_input_size_bound, program, primary_input);
    const ram_ppzksnark_proof<default_tinyram_ppzksnark_pp> proof =

...
#include <nil/crypto3/multiprecision/monty_exp.hpp>
...
*/
        ...
        secure_vector<uint8_t> rsa_private_key::private_key_bits() const {
            return der_encoder().start_cons(SEQUENCE).encode(static_cast<size_t>(0)).encode(m_n).encode(m_e).encode(
                    m_d).encode(m_p).encode(m_q).encode(m_d1).encode(m_d2).encode(m_c).end_cons().get_contents();
        }

        rsa_private_key::rsa_private_key(const algorithm_identifier &, const secure_vector<uint8_t> &key_bits) {
            ber_decoder(key_bits).start_cons(SEQUENCE).decode_and_check<size_t>(0,
                                                                                "Unknown PKCS #1 key format version").decode(
                    m_n).decode(m_e).decode(m_d).decode(m_p).decode(m_q).decode(m_d1).decode(m_d2).decode(
                    m_c).end_cons();
        }
        ...
        rsa_private_key::rsa_private_key(const cpp_int &prime1,
                                         ...
                                         const cpp_int &exp,
                                         const cpp_int &d_exp,
                                         const cpp_int &mod) : m_d{d_exp}, m_p{prime1},
                m_q{prime2}, m_d1{}, m_d2{}, m_c{inverse_mod(m_q, m_p)} {
...
*/
        bool rsa_private_key::check_key(random_number_generator &rng, bool strong) const {
            ...
            if (!miller_rabin_test(m_p, prob, rng) ||
                !miller_rabin_test(m_q, prob, rng)) {
                return false;
            }
...
*/
            ...
            protected:
                ...
                explicit rsa_private_operation(const rsa_private_key &rsa, random_number_generator &rng) : m_key(rsa),
                        m_mod_p(m_key.get_p()), m_mod_q(m_key.get_q()),
                        ...
                        m_blinder(m_key.get_n(), rng, [this](const cpp_int &k) {
            ...
            class rsa_signature_operation final
                    : public pk_operations::signature_with_emsa, private rsa_private_operation {
            public:
                ...
                rsa_signature_operation(const rsa_private_key &rsa, const std::string &emsa,
                                        random_number_generator &rng) : pk_operations::signature_with_emsa(emsa),
            ...
            class RSA_Decryption_Operation final
                    : public pk_operations::decryption_with_eme, private rsa_private_operation {
            public:

                RSA_Decryption_Operation(const rsa_private_key &rsa, const std::string &eme,
                                         random_number_generator &rng) : pk_operations::decryption_with_eme(eme),
                        rsa_private_operation(rsa, rng) {
                }
            ...
            class RSA_KEM_Decryption_Operation final
                    : public pk_operations::kem_decryption_with_kdf, private rsa_private_operation {
            public:

                RSA_KEM_Decryption_Operation(const rsa_private_key &key, const std::string &kdf,
                                             random_number_generator &rng) : pk_operations::kem_decryption_with_kdf(
                        kdf), rsa_private_operation(key, rng) {
                }
...
*/
            ...
            public:
                explicit RSA_Public_Operation(const rsa_public_key &rsa) : m_n(rsa.get_n()), m_e(rsa.get_e()),
            ...
            class RSA_Encryption_Operation final
                    : public pk_operations::encryption_with_eme, private RSA_Public_Operation {
            public:
            ...
            class RSA_Verify_Operation final
                    : public pk_operations::verification_with_emsa, private RSA_Public_Operation {
            public:
            ...
            class RSA_KEM_Encryption_Operation final
                    : public pk_operations::kem_encryption_with_kdf, private RSA_Public_Operation {
            public:
            ...
            private:
                void raw_kem_encrypt(secure_vector<uint8_t> &out_encapsulated_key,
                    ...
                    const cpp_int r = cpp_int::random_integer(rng, 1,
                                                                                                            get_n());
        ...
        std::unique_ptr<pk_operations::encryption> rsa_public_key::create_encryption_op(
        ...
        std::unique_ptr<pk_operations::kem_encryption> rsa_public_key::create_kem_encryption_op(
                random_number_generator & /*random*/, const std::string &params, const std::string &provider) const {
            if (provider == "core" || provider.empty()) {
                return std::unique_ptr<pk_operations::kem_encryption>(new RSA_KEM_Encryption_Operation(*this, params));
            }
            throw Provider_Not_Found(algo_name(), provider);
        }

        std::unique_ptr<pk_operations::verification> rsa_public_key::create_verification_op(const std::string &params,
        ...
        std::unique_ptr<pk_operations::decryption> rsa_private_key::create_decryption_op(random_number_generator &rng,
        ...
        std::unique_ptr<pk_operations::kem_decryption> rsa_private_key::create_kem_decryption_op(
        ...
        std::unique_ptr<pk_operations::signature> rsa_private_key::create_signature_op(random_number_generator &rng,
                                                                                       const std::string &params,
                                                                                       const std::string &provider) const {
#if defined(CRYPTO3_HAS_OPENSSL)
            if(provider == "openssl" || provider.empty())

...
namespace nil {
    namespace crypto3 {
        ...
        McEliece_PrivateKey::McEliece_PrivateKey(polyn_gf2m const &goppa_polyn,
                                                 ...
                                                 std::vector<uint8_t> const &public_matrix) : McEliece_PublicKey(
                public_matrix, goppa_polyn.get_degree(), inverse_support.size()), m_g(goppa_polyn),
                m_sqrtmod(square_root_matrix), m_Linv(inverse_support), m_coeffs(parity_check_matrix_coeffs),
        ...
        std::vector<uint8_t> McEliece_PublicKey::public_key_bits() const {
            return der_encoder().start_cons(SEQUENCE).start_cons(SEQUENCE).encode(
                    static_cast<size_t>(get_code_length())).encode(static_cast<size_t>(get_t())).end_cons().encode(
                    m_public_matrix, OCTET_STRING).end_cons().get_contents_unlocked();
        }
        ...
        McEliece_PublicKey::McEliece_PublicKey(const std::vector<uint8_t> &key_bits) {
            ...
            size_t n;
            size_t t;
            dec.start_cons(SEQUENCE).start_cons(SEQUENCE).decode(n).decode(t).end_cons().decode(m_public_matrix,
                                                                                                OCTET_STRING).end_cons();
            m_t = t;
            m_code_length = n;
        }
        ...
        secure_vector <uint8_t> McEliece_PrivateKey::private_key_bits() const {
            der_encoder enc;
            enc.start_cons(SEQUENCE).start_cons(SEQUENCE).encode(static_cast<size_t>(get_code_length())).encode(
                    static_cast<size_t>(get_t())).end_cons().encode(m_public_matrix, OCTET_STRING).encode(m_g.encode(),
        ...
        McEliece_PrivateKey::McEliece_PrivateKey(const secure_vector <uint8_t> &key_bits) {
            ...
            ber_decoder dec = dec_base.start_cons(SEQUENCE).start_cons(SEQUENCE).decode(n).decode(t).end_cons().decode(
                    m_public_matrix, OCTET_STRING).decode(enc_g, OCTET_STRING);
        ...
        namespace {
            ...
            public:
                ...
                MCE_KEM_Encryptor(const McEliece_PublicKey &key, const std::string &kdf) : kem_encryption_with_kdf(kdf),
                        m_key(key) {
                }
            ...
            public:
                ...
                MCE_KEM_Decryptor(const McEliece_PrivateKey &key, const std::string &kdf) : kem_decryption_with_kdf(
                        kdf), m_key(key) {
                }
        ...
        std::unique_ptr<pk_operations::kem_decryption> McEliece_PrivateKey::create_kem_decryption_op(
                RandomNumberGenerator & /*random*/, const std::string &params, const std::string &provider) const {
            if (provider == "core" || provider.empty()) {

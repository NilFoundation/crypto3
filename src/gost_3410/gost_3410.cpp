...
namespace nil {
    namespace crypto3 {
        ...
        algorithm_identifier gost_3410_public_key::get_algorithm_identifier() const {
            std::vector<uint8_t> params = der_encoder().start_cons(SEQUENCE).encode(
                    domain().get_curve_oid()).end_cons().get_contents_unlocked();
...
*/
            ...
            secure_vector <uint8_t> GOST_3410_Signature_Operation::raw_sign(const uint8_t msg[], size_t msg_len,
                ...
                const boost::multiprecision::cpp_int s = m_group.mod_order(
                        m_group.multiply_mod_order(r, m_x) + m_group.multiply_mod_order(k, e));

                if (r == 0 || s == 0) {
                    throw Internal_Error("GOST 34.10 signature generation failed, r/s equal to zero");
                }

                return boost::multiprecision::cpp_int::encode_fixed_length_int_pair(s, r, m_group.get_order_bytes());
            }
...
*/
        ...
        std::unique_ptr<pk_operations::verification> gost_3410_public_key::create_verification_op(const std::string &params,
                                                                                          const std::string &provider) const {
            if (provider == "core" || provider.empty()) {
                return std::unique_ptr<pk_operations::verification>(new GOST_3410_Verification_Operation(*this, params));
            }
            throw Provider_Not_Found(algo_name(), provider);
        }

        std::unique_ptr<pk_operations::signature> gost_3410_private_key::create_signature_op(random_number_generator & /*random*/,

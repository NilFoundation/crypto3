...
namespace nil {
    namespace crypto3 {
        ...
        bool sm2_signature_private_key::check_key(random_number_generator &rng, bool strong) const {
            ...
            return key_pair::signature_consistency_check(rng, *this, "SM3");
        }
...
*/
            ...
            public:
                ...
                sm2_signature_operation(const sm2_signature_private_key &sm2, const std::string &ident,
                                        const std::string &hash) : m_group(sm2.domain()), m_x(sm2.private_value()),
                        m_da_inv(sm2.get_da_inv()), m_hash(HashFunction::create_or_throw(hash)) {
            ...
            secure_vector<uint8_t> sm2_signature_operation::sign(random_number_generator &rng) {
                ...
                const boost::multiprecision::cpp_int r = m_group.mod_order(m_group.blinded_base_point_multiply_x(k, rng, m_ws) + e);
                const boost::multiprecision::cpp_int s = m_group.multiply_mod_order(m_da_inv, (k - r * m_x));

                // prepend ZA for next signature if any
                m_hash->update(m_za);

                return boost::multiprecision::cpp_int::encode_fixed_length_int_pair(r, s, m_group.get_order().bytes());
            }
...
*/
            ...
            public:
                sm2_verification_operation(const sm2_signature_public_key &sm2, const std::string &ident,
                                           const std::string &hash) : m_group(sm2.domain()),
        ...
        std::unique_ptr<pk_operations::verification> sm2_signature_public_key::create_verification_op(

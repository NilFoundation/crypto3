...
namespace nil {
    namespace crypto3 {
        ...
        namespace {
            ...
            public:
                SM2_Encryption_Operation(const sm2_encryption_public_key &key, const std::string &kdf_hash) : m_group(
                        key.domain()), m_mul_public_point(key.public_point()), m_kdf_hash(kdf_hash) {
                }
                ...
                secure_vector <uint8_t> encrypt(const uint8_t msg[], size_t msg_len,
                    ...
                    const secure_vector <uint8_t> kdf_output = kdf->derive_key(msg_len, kdf_input.data(),
                                                                               kdf_input.size());
                    ...
                    return der_encoder().start_cons(SEQUENCE).encode(x1).encode(y1).encode(C3, OCTET_STRING).encode(
                            masked_msg, OCTET_STRING).end_cons().get_contents();
                }
            ...
            public:
                SM2_Decryption_Operation(const sm2_encryption_private_key &key, random_number_generator &rng,
                                         const std::string &kdf_hash) : m_key(key), m_rng(rng), m_kdf_hash(kdf_hash) {
                }
                ...
                secure_vector <uint8_t> decrypt(uint8_t &valid_mask, const uint8_t ciphertext[],
                    ...
                    ber_decoder(ciphertext, ciphertext_len).start_cons(SEQUENCE).decode(x1).decode(y1).decode(C3,
                                                                                                              OCTET_STRING).decode(
                            masked_msg, OCTET_STRING).end_cons().verify_end();
                    ...
                    const secure_vector <uint8_t> kdf_output = kdf->derive_key(masked_msg.size(), kdf_input.data(),
                                                                               kdf_input.size());
        ...
        std::unique_ptr<pk_operations::decryption> sm2_encryption_private_key::create_decryption_op(random_number_generator &rng,
                                                                                            const std::string &params,
                                                                                            const std::string &provider) const {
            if (provider == "core" || provider.empty()) {
                const std::string kdf_hash = (params.empty() ? "SM3" : params);
                return std::unique_ptr<pk_operations::decryption>(new SM2_Decryption_Operation(*this, rng, kdf_hash));
            }

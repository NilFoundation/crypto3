...
*/
            ...
            secure_vector <uint8_t> ECKCDSA_Signature_Operation::raw_sign(const uint8_t msg[], size_t,
                ...
                const boost::multiprecision::cpp_int k_times_P_x = m_group.blinded_base_point_multiply_x(k, rng, m_ws);
...
*/
        ...
        std::unique_ptr<pk_operations::verification> ECKCdsa_public_key::create_verification_op(const std::string &params,
                                                                                        const std::string &provider) const {
            if (provider == "core" || provider.empty()) {
                return std::unique_ptr<pk_operations::verification>(new ECKCDSA_Verification_Operation(*this, params));
            }
            throw Provider_Not_Found(algo_name(), provider);
        }

        std::unique_ptr<pk_operations::signature> eckcdsa_private_key::create_signature_op(random_number_generator & /*random*/,

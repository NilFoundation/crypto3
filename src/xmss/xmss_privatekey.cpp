...
#include <nil/crypto3/internal/xmss_signature_operation.h>
#include <cmath>
...
namespace nil {
    namespace crypto3 {
        ...
        XMSS_PrivateKey::XMSS_PrivateKey(const secure_vector <uint8_t> &raw_key) : XMSS_PublicKey(unlock(raw_key)),
            ...
            BOOST_ASSERT_MSG(
                    sizeof(size_t) >= std::ceil(static_cast<float>(XMSS_PublicKey::m_xmss_params.tree_height()) / 8.f),
        ...
        void XMSS_PrivateKey::tree_hash_subtree(secure_vector <uint8_t> &result, size_t start_idx,
            ...
            nodes(target_node_height + 1, secure_vector<uint8_t>(XMSS_PublicKey::m_xmss_params.element_size()));
        ...
        std::unique_ptr<pk_operations::signature> XMSS_PrivateKey::create_signature_op(RandomNumberGenerator &,
                                                                                const std::string &,

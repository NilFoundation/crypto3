...
*/
            std::vector<uint8_t> encode_pbes2_params(const std::string &cipher, const std::string &prf,
                                                     ...
                                                     size_t key_length) {
                return der_encoder().start_cons(SEQUENCE).encode(algorithm_identifier("PKCS5.PBKDF2",
                                                                                     der_encoder().start_cons(
                                                                                             SEQUENCE).encode(salt,
                                                                                                              OCTET_STRING).encode(
                                                                                             iterations).encode(
                                                                                             key_length).encode_if(
                                                                                             prf != "HMAC(SHA-160)",
                                                                                             algorithm_identifier(prf,
                                                                                                                 algorithm_identifier::USE_NULL_PARAM)).end_cons().get_contents_unlocked())).encode(
                        algorithm_identifier(cipher, der_encoder().encode(iv,
                                                                         OCTET_STRING).get_contents_unlocked())).end_cons().get_contents_unlocked();
            }

/*
* PKCS#5 v2.0 PBE encryption
*/
            std::pair<algorithm_identifier, std::vector<uint8_t>> pbes2_encrypt_shared(
                    const secure_vector<uint8_t> &key_bits, const std::string &passphrase,
                    size_t *msec_in_iterations_out, size_t iterations_if_msec_null, const std::string &cipher,
                    const std::string &digest, RandomNumberGenerator &rng) {
                ...
                if (msec_in_iterations_out) {
                    ...
                    enc->set_key(pbkdf->derive_key(key_length, passphrase, salt.data(), salt.size(), msec,
                                                   iterations).bits_of());
        ...
        std::pair<algorithm_identifier, std::vector<uint8_t>> pbes2_encrypt(const secure_vector<uint8_t> &key_bits,
        ...
        std::pair<algorithm_identifier, std::vector<uint8_t>> pbes2_encrypt_msec(const secure_vector<uint8_t> &key_bits,
                                                                                const std::string &passphrase,
                                                                                std::chrono::milliseconds msec,
        ...
        std::pair<algorithm_identifier, std::vector<uint8_t>> pbes2_encrypt_iter(const secure_vector<uint8_t> &key_bits,
                                                                                const std::string &passphrase,
                                                                                size_t pbkdf_iter,
                                                                                const std::string &cipher,
                                                                                const std::string &digest,
        ...
        secure_vector<uint8_t> pbes2_decrypt(const secure_vector<uint8_t> &key_bits, const std::string &passphrase,
            ...
            ber_decoder(kdf_algo.get_parameters()).start_cons(SEQUENCE).decode(salt, OCTET_STRING).decode(
                    iterations).decode_optional(key_length, INTEGER, UNIVERSAL).decode_optional(prf_algo, SEQUENCE,
                                                                                                CONSTRUCTED,
                                                                                                algorithm_identifier(
                                                                                                        "HMAC(SHA-160)",
                                                                                                        algorithm_identifier::USE_NULL_PARAM)).end_cons();
